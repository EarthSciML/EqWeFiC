...
| Surface layer (sfclayrev) | φm/φh, bulk Richardson regimes, z0 over water, u*, exchange coefficients, 2 m/10 m diagnostics | `phys/physics_mmm/sf_sfclayrev.F90` | `local_scale/surface_layer_profile.esm` | instrumented (SCM dumps 7 calls + real64/real32 drivers; real32 replay bit-exact, 2026-09-05) |
...
| Land surface (slab) | 5-layer soil heat diffusion, surface energy balance | `phys/module_sf_slab.F` | — | instrumented (SCM dumps + SLAB1D diagnostics; real64/real32 drivers; dt sensitivity 1–10 %; 2026-09-05) |
...
| Microphysics (WSM6) | saturation adjustment; warm rain (praut, pracw, prevp); ice processes; melting/freezing; sedimentation | `phys/physics_mmm/mp_wsm6.F90` | — | physics (Spike C, 2026-09-05): `components/atmospheric_dynamics/wsm6/{saturation,warm_rain}.esm` (+ `wsm6_parameters`, `microphysics_templates`; `lib/wrf_thermo.esm` saturation templates, `lib/wrf_air_properties.esm`), 194/194 `./esm test` and Python binding; warm rain (praut, pracw, prevp) and saturation only; ice/melting/sedimentation/saturation-adjustment not started |
| SW radiation (Dudhia) | clear-sky absorption/scattering, cloud albedo/absorption, column integrals | `phys/module_ra_sw.F` | — | physics (Spike B, 2026-09-05): `components/atmospheric_radiation/dudhia_sw/dudhia_sw.esm` (+ `dudhia_sw_parameters`, `lacis_hansen_absorptance`, `stephens_cloud_tables` function tables), 174/174 `./esm test`; 5 SCM regimes + 1 Fortran-generated synthetic stratus regime; the >99 % single-layer depletion renormalisation is not transcribed |
| LW radiation (RRTM) | per-band optical depth, Planck, up/down sweeps, heating rate | `phys/module_ra_rrtm.F` | — | instrumented (SCM dumps + RRTM column internals and level fluxes; no standalone driver, real32 only; 2026-09-05) |
